...
test.group('analytics custom range', (group) => {
  ...
  test('applies and preserves a responsive custom date and time range', async ({
    ...
    await page.getByRole('radio', { name: 'Custom' }).click()
    const editedFoldDialog = page.getByRole('dialog', { name: 'Custom analytics range' })
    await editedFoldDialog.getByLabel('Start time').fill('02:35')
    await editedFoldDialog.getByLabel('Start time').fill('02:30')
    await editedFoldDialog.getByLabel('End time').fill('02:25')
    await editedFoldDialog.getByLabel('End time').fill('02:30')
    await editedFoldDialog.getByRole('button', { name: 'Apply range' }).click()
    await editedFoldDialog.waitFor({ state: 'hidden' })
    const reappliedFoldUrl = new URL(page.url())
    assert.equal(reappliedFoldUrl.searchParams.get('start'), '2026-10-25T00:30Z')
    assert.equal(reappliedFoldUrl.searchParams.get('end'), '2026-10-25T01:30Z')
  })
})
